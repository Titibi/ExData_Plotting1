...
#Plot 1
hist(hpc$Global_active_power,xlab="Global Active Power (kilowatts)", main = "Global Active Power (kilowatts)",col = "RED")
dev.copy(png, file="plot1.png", width=480, height=480)
dev.off()

# Plot 2
with(hpc,plot(as.POSIXct(as.character(paste(hpc$Date,hpc$Time), "%d/%m/%Y %H:%M:%S")),hpc$Global_active_power, type = 'l',xlab = ' ', ylab = "Global Active Power (kilowatts)"))
...
#Plot 4
par(mfrow=c(2,2))
with(hpc,plot(as.POSIXct(as.character(paste(hpc$Date,hpc$Time), "%d/%m/%Y %H:%M:%S")),hpc$Global_active_power, type = 'l',xlab = ' ', ylab = "Global Active Power"))
